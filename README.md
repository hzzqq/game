...
> **迭代20次·第二轮（2026-07-25）**：新一轮 20 批（每批 5 款，共 100 槽位）——先给此前缺 confetti 的 59 款补齐胜利/里程碑纸屑（达成 165/165 全覆盖），再对已覆盖的 40 款做「手感深化」：在关键反馈时刻注入 `Juice.shake` 屏震 + `Juice.burst` 粒子爆发。统一范式：`_fxShakes/_fxBursts` 计数器无条件递增、仅 `Juice.*` 调用受 `typeof Juice!=='undefined'` 守卫，坐标全为确定性表达式（不增全局随机消耗），`newGame` 归零、`window.__t` 暴露 `fxShakes/fxBursts` 只读钩子，避开各款已有 Juice 调用点防双触发。全程未改任何玩法/计分/状态机。逻辑单测 4947 → 5443 全绿，静态检测 953/953，电池扫描 165 款 0 缺陷。
